...
from conceptgraph.cg_process.obj_edge_processor import ObjEdgeProcessor, FeatureMergeDataset, ConfigDict, ObjFeatureGenerator
...
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.config_dict = ConfigDict(
            dataset_config = "/home/lg1/lujia/VLN_HGT/vlnce_baselines/m2g_config_files/dataset_r2r_finetune.yaml",
            detection_config= "/home/lg1/lujia/VLN_HGT/vlnce_baselines/m2g_config_files/detection_r2r_finetune.yaml",
            merge_config= "/home/lg1/lujia/VLN_HGT/vlnce_baselines/m2g_config_files/merge_r2r_finetune.yaml"
            )
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        ### M2G
        # path_vp_envs = [[]] * self.envs.num_envs # Don't ever using this to init the path_vp_envs. Each list in the path_vp_envs is the same list. It will be operated together in the loop.
        ...
        for stepk in range(self.max_len):
            ...
            for i in range(self.envs.num_envs):
                ...
                # closest_vp_i = self.obj_edge_processor.find_similar_vp(cur_scan_i, cur_pos[i])
                
                # cur_scan.append(cur_scan_i)
                # closest_vp.append(closest_vp_i)
                # path_vp_envs[i].append(closest_vp_i)
                
                # _objs, _objs_feature, _objs_pose, _edges_objects, _edges_relationship = self.obj_edge_processor.get_merge_feature_edges_relationship(cur_scan[i], path_vp_envs[i])
                # filtered_objs.append(_objs)
                # filtered_objs_feature.append(_objs_feature)
                # filtered_objs_pose.append(_objs_pose)
                # edges_objects_extend.append(_edges_objects)
                ...
                # Cleaning my workspace, I want to be a thoughtful programmer
                del dataset
                gc.collect()
