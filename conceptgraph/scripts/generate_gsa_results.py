...
torch.set_grad_enabled(False) # M2G
...
if __name__ == "__main__":
    ...
    scanvp_list = load_viewpoint_ids('/home/lg1/peteryu_workspace/BEV_HGT_VLN/datasets/R2R/connectivity')
    
    scan_old = scanvp_list[0][0]
    vp_traj = []
    ...
    for scan, vp in scanvp_list:
        if scan == scan_old:
            vp_traj.append(vp)
        if scan != scan_old:
            # update scene_id to scan
            args.scene_id = scan_old
            args.trajectory = vp_traj
            print("Processing the scene: ", scan_old)
            main(args)
            processed_scans.append(scan_old)
            print("Processed scenes: ", processed_scans)
            vp_traj = []
            vp_traj.append(vp)
            scan_old = scan
...
# cv2.imread('/data/vln_datasets/preprocessed_data/preprocessed_habitiat_R2R/2azQ1b91cZZ/color_image/0a9f30bd318e40de89f71e4bf6987358_i_1.jpg')